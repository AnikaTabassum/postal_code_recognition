...
class segmentation:
	...
	def segment_the_image(self, filename):

		fullFileName=os.getcwd()+"/dataset/cropped_and_resized/"+ filename
		# Read the image
		...
		for c in contours:
			...
			print("width---",w,"-----Height----", h, "diff ", w-h)
			if (abs(w-h) < 40) and w >40  and h >40 and h<90:
				print("ind", idx)
				idx += 1
				new_img = img[y:y+h, x:x+w]
				filewrite= os.getcwd()+ "/dataset/segmented/"+ filename
				# If the box height is greater then 20, widht is >80, then only save it as a box in "cropped/" folder.
				cv2.imwrite(filewrite+str(idx) + '.png', new_img)
			# if (w > 80 and h > 20) and w > 3*h:
			#     idx += 1
			#     new_img = img[y:y+h, x:x+w]
			#     cv2.imwrite("cropped/"+str(idx) + '.png', new_img)

# segmentation().segment_the_image("2_a.jpg")